    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-violet-50">
      ...
      <article className="max-w-4xl mx-auto px-6 py-16 space-y-16">
        ...
        >
          ...
          <CodeBlock language="typescript">{`// ── 基礎泛型（你可能已知道）──
...
getLength(42);           // ❌ TypeScript 錯誤！number 沒有 length`}</CodeBlock>
          ...
          <CodeBlock language="typescript">{`// 從物件中安全地取值
...
// 型別：{ id: number; name: string }（不包含 email）`}</CodeBlock>
        ...
        >
          ...
          <CodeBlock language="typescript">{`// 基本語法：T extends U ? X : Y
...
// = (string | number)[]`}</CodeBlock>
          ...
          <CodeBlock language="typescript">{`// 從函式型別中提取回傳型別（ReturnType 的實作原理）
...
type H2 = ArrayElement<[1, 'a', true]>;    // 1 | 'a' | true（Tuple 的聯合）`}</CodeBlock>
        ...
        >
          ...
          <CodeBlock language="typescript">{`// Mapped Type 的基本語法：[K in keyof T]
...
// { name: string; email: string }`}</CodeBlock>
          ...
          <CodeBlock language="typescript">{`// 把 key 改成 Getter 形式
...
// { id: number }（name 和 email 因為含 null/undefined 被過濾掉）`}</CodeBlock>
        </motion.section>
        ...
        >
          ...
          <CodeBlock language="typescript">{`// 基本用法：組合 string literal
...
// }`}</CodeBlock>
        ...
        >
          ...
          <CodeBlock language="typescript">{`// ── 以下是 TypeScript 內建 Utility Types 的實作原理 ──
...
type G2 = Exclude<string | number | null, null>;   // string | number`}</CodeBlock>
...
user.name = 'hacked';  // ❌ TypeScript 錯誤！回傳值是 readonly`}</CodeBlock>
        </motion.section>
        ...
        >
          ...
          <CodeBlock language="typescript">{`// ── ❌ 容易出 bug 的傳統做法 ──
type LoadingState = {
  isLoading: boolean;
  data: User | null;
  error: string | null;
